...
async def get_dashboard_metrics(request: DateRangeRequest):
    ...
    - brends: список для фильтра брендов (опционально)
    ...
    try:
        ...
        logger.info(
            ...
            f"filters: brends={request.brends}, category={request.category}, sa_name={request.sa_name}"
        )
        ...
        primary_report = get_metrics_for_period_from_report(
            ...
            brands=request.brends,
            categories=request.category,
            sa_names=request.sa_name
        )
        compare_report = None
        if request.compare:
...
# ═══════════════════════════════════════════════════════════════
# ЭНДПОИНТ 3: POST /api/dashboard/details
# ═══════════════════════════════════════════════════════════════
